...
import {
  MetricReporter,
  e as TelemetryNamespace
} from "./telemetry/definitions/index.js";
import {
  m as mcpLogger
...
import {
  b as getActiveMcpConfigLocation,
  g as getHomeKiroPath
} from "./paths-CZlww8F7.js";
import {
  z
} from "zod";
...
async function showUnapprovedVarsWarningIfNeeded() {
  if (globalUnapprovedVarsFound.size > 0 && !hasShownWarningThisSession) {
    ...
    if (selection === "Approve & Allow") {
      ...
      await vscode.workspace.getConfiguration("kiroAgent").update(APPROVED_ENV_VARS_KEY, updatedApproved, vscode
...
class MCPConnection {
  ...
  async connectClient(signal, fallbackToStreamableHttp = false) {
    ...
    try {
      ...
      } else if (error instanceof UnauthorizedError && this.oauthRedirectServer && (
          transport instanceof StreamableHTTPClientTransport || transport instanceof SSEClientTransport)) {
        const {
          code,
          state
