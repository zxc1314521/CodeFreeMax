...
class IDCAuthProvider {
  constructor() {
    ...
    const scopePrefix = vscode.workspace.getConfiguration("codewhisperer.config").get("scopePrefix") ??
      "codewhisperer";
  ...
  async handleProfiles(token) {
    ...
    if (profiles) {
      ...
      if (profiles.length > 1) {
        vscode.commands.executeCommand("kiro.profiles.showProfileSelector", {
          isDismissible: false
        });
      }
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      uriHandler = uriEventHandler.onUri((uri) => {
        ...
        try {
          ...
          reject(new UnexpectedIssueError(
            `SocialAuth: Unknown code response error: ${getUnknownErrorDetails(error)}`));
        } finally {
          this._cancelAuth = void 0;
          uriHandler.dispose();
        }
