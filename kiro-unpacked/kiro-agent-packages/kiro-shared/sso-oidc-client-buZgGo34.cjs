...
const Metrics$1 = new telemetry_definitions_index.MetricReporter(telemetry_definitions_index.TelemetryNamespace.Auth, "auth-service-client");
...
function logCommonErrorDetails$1(error, span2) {
  ...
  try {
    if (error instanceof axios.AxiosError && error.response) {
      ...
      span2.setAttribute("errorDescription", error.message || ((_a = error.cause) == null ? void 0 : _a.message) || getUnknownErrorDetails(error));
    } else {
...
const Metrics = new telemetry_definitions_index.MetricReporter(telemetry_definitions_index.TelemetryNamespace.Auth, "sso-oidc-client");
