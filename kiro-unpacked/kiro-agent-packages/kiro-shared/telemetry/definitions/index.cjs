...
const KIRO_CLIENT_VERSION = `${semanticVersion$1 == null ? void 0 : semanticVersion$1.major}.${semanticVersion$1 == null ? void 0 : semanticVersion$1.minor}`;
...
function recordOnboardingStep(type) {
  try {
    ...
    ignore.logger.error("Failed to record feature latency: ", e);
  }
}
...
function recordAuthFromSource(source) {
  try {
    ...
    ignore.logger.error("Failed to record auth login metrics: ", e);
  }
}
...
const _MetricReporter = class _MetricReporter {
  ...
  static async handlePeriodicReporters() {
    ...
    for (const [trackerName, handler] of this.periodicReporters) {
      ...
      for (const metricName of Object.keys(report)) {
        const attributes = { ...DefaultAttributes, Periodic: trackerName };
  ...
  wrapMetrics(operationName, fn) {
    const baseAttributes = { ...DefaultAttributes, Operation: operationName };
    return wrapCallback({
      callback: fn,
      before: () => {
        const attributes = {
      ...
      failure: (error) => {
        const errorType = ignore.mapUnknownToErrorType(error);
        const tracker = this.getOperationTrackers(operationName, errorType);
        const attributes = {
          ...baseAttributes,
          ...ContextPropagation.getTelemetryAttributes()
        };
        const isAbort = ignore.isAbortError(error);
        const isBlocked = ignore.isBlockedAccessError(error);
        if (isAbort) {
  ...
  wrapTrace(traceName, fn, getAdditionalDimensions) {
    const baseAttributes = { ...DefaultAttributes, Operation: traceName };
  ...
  createTrace(traceName) {
    const attributes = { ...DefaultAttributes, Operation: traceName };
    const fullTraceName = `${this.scope}.${traceName}`;
    const tracer = api.trace.getTracer(exports.TelemetryNamespace.Application, APPLICATION_VERSION);
    let span = void 0;
    return {
...
const KiroClientVersion = `${semanticVersion == null ? void 0 : semanticVersion.major}.${semanticVersion == null ? void 0 : semanticVersion.minor}`;
const KiroClientVersionStr = `${semanticVersion == null ? void 0 : semanticVersion.major}-${semanticVersion == null ? void 0 : semanticVersion.minor}`;
...
ignore.logger.info(`Loading telemetry with client attributes`, DefaultAttributes);
...
const TOOL_CONFIG = {
  [
    "WriteFile"
    /* WriteFile */
  ]: { name: "write_file", description: "Tracks write file tool" },
  [
    "GrepSearch"
    /* GrepSearch */
  ]: { name: "grep_search", description: "Tracks grep search tool" },
  [
    "FileSearch"
    /* FileSearch */
  ]: { name: "file_search", description: "Tracks file search tool" },
  [
    "ReplaceText"
    /* ReplaceText */
  ]: { name: "replace_text", description: "Tracks replace text tool" },
  [
    "EditCode"
    /* EditCode */
  ]: { name: "edit_code", description: "Tracks edit code tool" },
  [
    "SelfLabel"
    /* SelfLabel */
  ]: { name: "self_label", description: "Tracks self label tool" },
  [
    "AppendLines"
    /* AppendLines */
  ]: { name: "append_lines", description: "Tracks append lines tool" },
  [
    "DeleteFile"
    /* DeleteFile */
  ]: { name: "delete_file", description: "Tracks delete file tool" },
  [
    "ExecuteShell"
    /* ExecuteShell */
  ]: { name: "execute_shell", description: "Tracks execute shell tool" },
  [
    "ControlProcess"
    /* ControlProcess */
  ]: { name: "control_process", description: "Tracks control process tool" },
  [
    "ListProcesses"
    /* ListProcesses */
  ]: { name: "list_processes", description: "Tracks list processes tool" },
  [
    "GetProcessOutput"
    /* GetProcessOutput */
  ]: { name: "get_process_output", description: "Tracks get process output tool" },
  [
    "OpenFolder"
    /* OpenFolder */
  ]: { name: "open_folder", description: "Tracks open folder tool" },
  [
    "FindFiles"
    /* FindFiles */
  ]: { name: "find_files", description: "Tracks find files tool" },
  [
    "GetUserInput"
    /* GetUserInput */
  ]: { name: "get_user_input", description: "Tracks get user input tool including fix suggestions" },
  [
    "ReadFile"
    /* ReadFile */
  ]: { name: "read_file", description: "Tracks read file tool" },
  [
    "ReadMultipleFiles"
    /* ReadMultipleFiles */
  ]: { name: "read_multiple_files", description: "Tracks read multiple files tool" },
  [
    "NativeSearch"
    /* NativeSearch */
  ]: { name: "native_search", description: "Tracks native search tool" },
  [
    "MCPWrapper"
    /* MCPWrapper */
  ]: { name: "mcp_wrapper", description: "Tracks any custom MCP tool call" },
  [
    "TaskStatus"
    /* TaskStatus */
  ]: { name: "task_status", description: "Tracks the task status tool" },
  [
    "PBTStatus"
    /* PBTStatus */
  ]: { name: "pbt_status", description: "Tracks the PBT status tool" },
  [
    "GetDiagnostics"
    /* GetDiagnostics */
  ]: { name: "get_diagnostics", description: "Tracks get diagnostics tool" },
  [
    "Prework"
    /* Prework */
  ]: { name: "prework", description: "Tracks the prework tool" },
  [
    "ReadCode"
    /* ReadCode */
  ]: { name: "read_code", description: "Tracks read code tool" },
  [
    "InvokeSubAgent"
    /* InvokeSubAgent */
  ]: { name: "invoke_sub_agent", description: "Tracks invoke sub-agent tool" },
  [
    "ReportProgress"
    /* ReportProgress */
  ]: { name: "report_progress", description: "Tracks report progress tool" },
  [
    "KiroPowers"
    /* KiroPowers */
  ]: { name: "kiro_powers", description: "Tracks Kiro Powers tool" },
  [
    "SubagentResponse"
    /* SubagentResponse */
  ]: { name: "subagent_response", description: "Tracks subagent response tool" },
  [
    "WebFetch"
    /* WebFetch */
  ]: { name: "web_fetch", description: "Tracks web fetch tool" }
};
const ToolUsage = new MetricReporter(exports.TelemetryNamespace.Tool, "tools");
...
function initializeCounters$1() {
  const toolUsageMeter = api.metrics.getMeterProvider().getMeter(MetricNamespace.Tool);
...
class ToolRecorder {
   ...
   */
  static start(tool) {
    ToolUsage.reportCountMetrics({ toolUsage: 1 }, { Operation: `useTool.${tool}` });
  ...
  recordHistogram(metricName, value, dimensions) {
    ...
    ToolUsage.reportHistogramMetrics({ [fullMetricName]: value, [metricName]: value }, dimensions);
  }
  /**
...
function recordBashToolEvent(type) {
  try {
    ...
    ignore.logger.error("Failed to record bash execution outcome: ", e);
  }
}
...
exports.Tool = Tool;
exports.ToolRecorder = ToolRecorder;
