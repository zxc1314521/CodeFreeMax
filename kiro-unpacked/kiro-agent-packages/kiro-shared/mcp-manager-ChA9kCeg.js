...
import { m as mcpLogger } from "./ignore-B7xGFdAM.js";
...
import { b as getActiveMcpConfigLocation, g as getHomeKiroPath } from "./paths-CZlww8F7.js";
import { z } from "zod";
...
const MCPJsonConfigSchema = z.object({
  mcpServers: z.record(MCPOptionsSchema),
  powers: z.object({
    mcpServers: z.record(MCPOptionsSchema)
  }).optional()
});
...
async function showUnapprovedVarsWarningIfNeeded() {
  if (globalUnapprovedVarsFound.size > 0 && !hasShownWarningThisSession) {
    ...
    mcpLogger.warn(`Found unapproved environment variables in MCP config: ${unapprovedVars.join(", ")}`);
    ...
    const selection = await vscode.window.showErrorMessage(message, "Approve & Allow");
    if (selection === "Approve & Allow") {
      const currentApproved = vscode.workspace.getConfiguration("kiroAgent").get(APPROVED_ENV_VARS_KEY, []);
      const updatedApproved = [.../* @__PURE__ */ new Set([...currentApproved, ...unapprovedVars])];
      await vscode.workspace.getConfiguration("kiroAgent").update(APPROVED_ENV_VARS_KEY, updatedApproved, vscode.ConfigurationTarget.Global);
      mcpLogger.info(`User approved environment variables: ${unapprovedVars.join(", ")}`);
      hasShownWarningThisSession = false;
      const configLocation = getActiveMcpConfigLocation(getWorkspaceDirs());
...
function loadPowersMcpConfig() {
  const userConfigPath = path.join(getHomeKiroPath(), "settings", "mcp.json");
  if (!fs.existsSync(userConfigPath)) {
    return null;
  }
  try {
    const rawContent = fs.readFileSync(userConfigPath, "utf-8");
    const parsed = JSONC.parse(rawContent);
      ...
      if (result.success) {
        ...
        Object.entries(result.data.mcpServers).forEach((entry) => {
          ...
          if (validationResult.isValid) {
            ...
            mcpLogger.warn(validationResult.invalidMessage || "", entry[0]);
    ...
    mcpLogger.warn("[Powers Debug] No powers.mcpServers section found in user mcp.json");
    return null;
  } catch (error) {
    const errorMsg = `Error loading powers config: ${error instanceof Error ? error.message : String(error)}`;
    mcpLogger.error("[Powers Debug] " + errorMsg);
...
function loadMcpConfig() {
  ...
  if (powerServers.length > 0) {
    mcpLogger.info(`[Powers Debug] Loaded ${powerServers.length} power MCP servers: ${powerServers.join(", ")}`);
  }
...
function expandEnvironmentVariables(value) {
  if (typeof value === "string") {
    const approvedVars = vscode.workspace.getConfiguration("kiroAgent").get(APPROVED_ENV_VARS_KEY, []);
    return value.replace(/\$\{([^}]+)\}/g, (match, varName) => {
      ...
      if (envValue !== void 0) {
        ...
        mcpLogger.debug(`Expanded environment variable: ${varName}`);
        return envValue;
      }
      return match;
    });
...
function loadIndividualMcpConfig(sourceName, path2) {
  ...
  try {
    const configContent = fs.readFileSync(path2, "utf8");
    const parsedJson = JSONC.parse(configContent);
...
async function surfaceConfigError(errorMsg, sourceName, configPath) {
  mcpLogger.error(errorMsg);
  mcpLogger.show();
  const selection = await vscode.window.showErrorMessage(errorMsg, "Ask Kiro", "Open Settings");
  if (selection === "Ask Kiro") {
    const message = `I'm getting this MCP configuration error: "${errorMsg}". Can you help me fix it?`;
    await vscode.commands.executeCommand("kiroAgent.userInputFocusNoSubmit", message);
  } else if (selection === "Open Settings") {
    if (sourceName === "workspace") {
      await vscode.commands.executeCommand("kiroAgent.openWorkspaceMcpConfig", false, configPath);
    } else {
      await vscode.commands.executeCommand("kiroAgent.openUserMcpConfig", false);
    }
  }
}
...
async function addMCPServerConfig(serverName, config) {
  await vscode.commands.executeCommand("kiroAgent.openUserMcpConfig");
  const userConfigLocation = getActiveMcpConfigLocation(getWorkspaceDirs());
  if (!userConfigLocation.userConfigPath) {
    void vscode.window.showErrorMessage("Failed to create User MCP server config");
    return;
  }
  ...
  if (serverName in currentConfig.mcpServers) {
    void vscode.window.showErrorMessage(`MCP Server ${serverName} already exists in user config`);
    return;
  }
  let newServerConfig;
  try {
    const parsed = JSONC.parse(config);
    ...
    void vscode.window.showErrorMessage(`Invalid MCP server config: ${e instanceof Error ? e.message : String(e)}`);
    return;
  }
  const newConfig = {
    ...currentConfig,
  ...
  fs.writeFileSync(userConfigLocation.userConfigPath, JSONC.stringify(newConfig, null, 2));
...
async function addMCPToolToAutoApproveConfig(serverName, toolName) {
  ...
  mcpLogger.info(addedMessage, serverName);
  const selection = await vscode.window.showInformationMessage(addedMessage, "Open Settings");
  if (selection === "Open Settings") {
    const matchingConfig = findConfigFileForServer(serverName);
    if (matchingConfig.sourceName === "workspace") {
      await vscode.commands.executeCommand("kiroAgent.openWorkspaceMcpConfig");
    } else {
      await vscode.commands.executeCommand("kiroAgent.openUserMcpConfig");
    }
  }
...
function applyServerConfigTransform(serverName, transform) {
  const matchingConfig = findConfigFileForServer(serverName);
  const rawContent = fs.readFileSync(matchingConfig.path, "utf-8");
  const parsed = JSONC.parse(rawContent);
  ...
  fs.writeFileSync(matchingConfig.path, JSONC.stringify(fullConfig, null, 2));
}
function findConfigFileForServer(serverName) {
  ...
  if (configLocation.userConfigPath) {
    ...
    const rawContent = fs.readFileSync(configLocation.userConfigPath, "utf-8");
    const parsed = JSONC.parse(rawContent);
    const fullConfig = parsed && typeof parsed === "object" ? parsed : {};
...
function resetApprovedEnvVars() {
  vscode.workspace.getConfiguration("kiroAgent").update(APPROVED_ENV_VARS_KEY, [], vscode.ConfigurationTarget.Global);
  hasShownWarningThisSession = false;
  mcpLogger.info("Reset approved environment variables list");
}
...
class MCPAuthProvider {
  ...
  async redirectToAuthorization(authorizationUrl) {
    try {
      if (this.startOAuth) {
        await vscode.env.openExternal(authorizationUrl.href);
      }
    } catch (error) {
...
class OAuthRedirectServer {
   ...
   */
  async start() {
    mcpLogger.info("Starting server for MCP OAuth redirect...");
    try {
      this.callbackUrl = await this.attemptServerStart();
      mcpLogger.info(`Server for MCP OAuth redirect started on ${this.callbackUrl}`);
   ...
   */
  async stop() {
    mcpLogger.info("Stopping server for MCP OAuth redirect");
    if (!this.server) {
      return;
    }
    return new Promise((resolve, reject) => {
...
class MCPConnection {
  ...
  constructStdioTransport(options, cwd) {
    ...
    if (transport.stderr !== null) {
      transport.stderr.on("data", (data) => {
        mcpLogger.warn(`Log from MCP Server: ${data}`, this.serverName);
      });
    }
    return transport;
  }
  async connectClient(signal, fallbackToStreamableHttp = false) {
    ...
    try {
      ...
      mcpLogger.info(`Connected to server with transport type: ${transportType}`, this.serverName);
    } catch (error) {
      if (error instanceof SseError && !fallbackToStreamableHttp) {
        this.connectPromise = null;
        await this.connectClient(signal, true);
      } else if (error instanceof UnauthorizedError && this.oauthRedirectServer && (transport instanceof StreamableHTTPClientTransport || transport instanceof SSEClientTransport)) {
        const { code, state } = await this.oauthRedirectServer.waitForCallback();
        if (state !== this.oauthProvider.state()) {
          throw new Error("Invalid OAuth state");
        }
        await transport.finishAuth(code);
        mcpLogger.info(`Received authorization code, completing OAuth`, this.serverName);
        this.connectPromise = null;
  ...
  async close() {
    try {
      ...
      mcpLogger.info("MCP connection closed successfully", this.serverName);
    } catch (error) {
      mcpLogger.warn(
    ...
    try {
      ...
      if (capabilities == null ? void 0 : capabilities.tools) {
        try {
          ...
          if (e instanceof McpError && e.code === ErrorCode.MethodNotFound.valueOf()) {
            mcpLogger.warn("MCP server does not support listing tools. Skipping...", serverName);
          } else {
            throw e;
          }
        }
      ...
      if (capabilities == null ? void 0 : capabilities.resources) {
        try {
          ...
          if (e instanceof McpError && e.code === ErrorCode.MethodNotFound.valueOf()) {
            mcpLogger.warn("MCP server does not support listing resources. Skipping...", serverName);
          } else {
            throw e;
          }
        }
      ...
      mcpLogger.error(`Error getting tools and resources from MCP server: ${errorMsg}`, serverName);
      throw error;
    } finally {
      if (this.oauthRedirectServer) {
...
const _MCPManagerSingleton = class _MCPManagerSingleton {
  constructor() {
    ...
    this.metricReporter = new MetricReporter(TelemetryNamespace.Application, "mcp");
  ...
  setConnectionState(serverName, connectionState) {
    ...
    vscode.commands.executeCommand("kiroAgent.views.mcpServerStatus.refresh");
  }
  /**
  ...
  registerServer(serverName, options, configuredIndex, useOAuth, onConnectionChange, cwd) {
    ...
    if (existingState) {
      if (existingState.status === "connecting" || existingState.status === "connected") {
        mcpLogger.info(`MCP server is already ${existingState.status}`, serverName);
        return;
      }
    }
    mcpLogger.info("Registering MCP server and starting connection", serverName);
  ...
  async startConnection(serverName, options, configuredIndex, useOAuth, onConnectionChange, cwd) {
    ...
    const timeoutId = setTimeout(() => {
      ...
      mcpLogger.error(timeoutMessage, serverName);
    ...
    try {
      ...
      if (!currentState || currentState.status === "disabled") {
        mcpLogger.warn("Connection disabled or removed during connection, removing...", serverName);
        await connection.close();
        return;
      }
      const taggedTools = this.updateToolTags(options.autoApprove, options.disabledTools, resourcesAndTools.tools);
      mcpLogger.info("Successfully connected and synced tools and resources for MCP server", serverName);
      this.setConnectionState(serverName, {
        status: "connected",
        connection,
        serverName,
      ...
      mcpLogger.error(`Error connecting to MCP server: ${errorMsg}`, serverName);
      if (failedStatus === "unauthorized") {
        ...
        const action = await vscode.window.showInformationMessage(message, "Authenticate");
  ...
  async reloadMcpConfig(mcpConfig, onConnectionChange, cwd) {
    var _a;
    if (!mcpConfig) {
      mcpLogger.info("No MCP servers configured, closing all existing connections");
    ...
    for (const serverName of serversWithConfigRemoved) {
      mcpLogger.info("Removing MCP server as it is no longer in the configuration", serverName);
      await this.removeConnection(serverName, void 0, void 0, onConnectionChange);
    }
    for (const { serverName, index } of serversWithConfigAdded) {
      const serverConfig = mcpConfig.mcpServers[serverName];
      if (!serverConfig.disabled) {
        mcpLogger.info("Adding new MCP server from updated configuration", serverName);
        this.registerServer(serverName, serverConfig, index, false, onConnectionChange, cwd);
      } else {
        mcpLogger.info("New MCP server from updated configuration is disabled", serverName);
    ...
    for (const { serverName, index } of serversWithConfigModified) {
      ...
      const connectionConfigChanged = JSON.stringify({ ...existingState == null ? void 0 : existingState.options, autoApprove: [], disabledTools: [] }) !== JSON.stringify({ ...newConfig, autoApprove: [], disabledTools: [] });
      if (connectionConfigChanged) {
        if (!newConfig.disabled) {
          mcpLogger.info("Configuration changed for MCP server, reconnecting", serverName);
          ...
          mcpLogger.info("Configuration disabled for MCP server, disconnecting", serverName);
          await this.removeConnection(serverName, newConfig, index, onConnectionChange);
        }
  ...
  async resetConnection(serverName, useOAuth = false) {
    ...
    if (!currentConnection) {
      mcpLogger.warn("No connection found", serverName);
      return;
    }
    mcpLogger.info("Reconnecting to server", serverName);
  ...
  static async reset() {
    ...
    try {
      ...
      mcpLogger.error(`Error removing connection for MCP server: ${e instanceof Error ? e.message : String(e)}`);
    }
...
export {
  MCPOptionsSchema as M,
  MCPJsonConfigSchema as a,
  MCPManagerSingleton as b,
  MCPConnection as c,
  addMCPServerConfig as d,
  enableMCPTools as e,
  formatToolName as f,
  disableMCPTools as g,
  addMCPToolToAutoApproveConfig as h,
  findConfigFileForServer as i,
  loadMcpConfig as l,
  mcpServerSources as m,
  resetApprovedEnvVars as r,
  setMCPServerDisabled as s
};
