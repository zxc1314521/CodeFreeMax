...
var __privateGet = (obj, member, getter) => (__accessCheck(obj, member, "read from private field"), getter ? getter
  .call(obj) : member.get(obj));
var __privateAdd = (obj, member, value) => member.has(obj) ? __typeError(
  "Cannot add the same private member more than once") : member instanceof WeakSet ? member.add(obj) : member.set(obj,
  value);
var __privateSet = (obj, member, value, setter) => (__accessCheck(obj, member, "write to private field"), setter ?
  setter.call(obj, value) : member.set(obj, value), value);
var _lastInstance;
...
const _AuthSSOServer = class _AuthSSOServer {
    ...
    return span.withSpan(telemetry_definitions_index.TelemetryNamespace.Auth, "auth-server.init", async () => {
      ...
      if (lastInstance !== void 0 && !lastInstance.closed) {
        try {
          ...
          ignore.logger.error("Failed to close already existing auth server in AuthSSOServer.init(): %s",
            error);
        }
      }
  ...
  /** Starts the server */
  start(ports) {
    return span.withSpan(telemetry_definitions_index.TelemetryNamespace.Auth, "auth-server.start", async (
    span2) => {
      let portIndex = 0;
  ...
  /** Closes the server */
  close() {
    return span.withSpan(telemetry_definitions_index.TelemetryNamespace.Auth, "auth-server.close", async (
    span2) => {
      try {
        span2.setAttribute("redirectUri", this.redirectUri);
      } catch (_e) {
  ...
  handleAuthentication(parameters, response) {
    ...
    if (error && errorDescription) {
      if (error === "access_denied") {
        ...
        this.handleRequestRejection(response, new ssoOidcClient.AuthProviderFailure(
  ...
  ensureCacheDirectory() {
    try {
      if (!fs__namespace.existsSync(this.cacheDirectory)) {
        fs__namespace.mkdirSync(this.cacheDirectory, {
          recursive: true
        });
      }
    } catch (_e) {
...
class IDCAuthProvider {
  constructor() {
    ...
    const scopePrefix = vscode__namespace.workspace.getConfiguration("codewhisperer.config").get("scopePrefix") ??
      "codewhisperer";
  ...
  async authenticate(options) {
    return span.withSpan(telemetry_definitions_index.TelemetryNamespace.Auth, "idc-provider.authenticate", async (
      span2) => {
      ...
      const token = this.tokenResponseToToken(response, this.getClientIdHash(startUrl), options.provider,
        region);
  ...
  async refreshToken(token) {
    return span.withSpan(telemetry_definitions_index.TelemetryNamespace.Auth, "idc-provider.refreshToken", async (
      span2) => {
  ...
  async deleteAccount(token) {
    return span.withSpan(telemetry_definitions_index.TelemetryNamespace.Auth, "idc-provider.deleteAccount", async (
      span2) => {
      span2.setAttribute("authProvider", token.provider);
      if (token.authMethod !== "IdC") {
        throw new ssoOidcClient.UnexpectedIssueError("IdC auth: invalid auth method");
  ...
  async handleProfiles(token) {
    ...
    if (profiles) {
      ...
      if (profiles.length > 1) {
        vscode__namespace.commands.executeCommand("kiro.profiles.showProfileSelector", {
          isDismissible: false
        });
      }
    }
...
class SocialAuthProvider {
    ...
    return new Promise((resolve, reject) => {
      ...
      uriHandler = uri.uriEventHandler.onUri((uri2) => {
        ...
        try {
          ...
          reject(new ssoOidcClient.UnexpectedIssueError(
            `SocialAuth: Unknown code response error: ${ssoOidcClient.getUnknownErrorDetails(error)}`));
        } finally {
          this._cancelAuth = void 0;
          uriHandler.dispose();
        }
   ...
   */
  async authenticate(options) {
    return span.withSpan(telemetry_definitions_index.TelemetryNamespace.Auth, "social-provider.authenticate", async (
      span2) => {
      span2.setAttribute("authProvider", options.provider);
      ...
      const codeVerifier = crypto.randomBytes(32).toString("base64url");
      const codeChallenge = crypto.createHash("sha256").update(codeVerifier).digest().toString("base64url");
      await this.authServiceClient.login({
        provider,
        redirectUri,
        codeChallenge,
        state
      });
      const code = await withTimeout(this.handleCodeResponse(state), REDIRECT_TIMEOUT, ssoOidcClient
        .SSORedirectTimeoutError);
   ...
   */
  async refreshToken(token) {
    return span.withSpan(telemetry_definitions_index.TelemetryNamespace.Auth, "social-provider.refreshToken", async (
      span2) => {
      span2.setAttribute("authProvider", token.provider);
      ...
      try {
        const token2 = await this.authServiceClient.refreshToken({
          refreshToken
        });
   ...
   */
  async logout(token) {
    return span.withSpan(telemetry_definitions_index.TelemetryNamespace.Auth, "social-provider.logout", async (
      span2) => {
        span2.setAttribute("authProvider", token.provider);
   ...
   */
  async deleteAccount(token) {
    return span.withSpan(telemetry_definitions_index.TelemetryNamespace.Auth, "social-provider.deleteAccount", async (
      span2) => {
      span2.setAttribute("authProvider", token.provider);
      return this.authServiceClient.deleteAccount(token.accessToken);
    });
