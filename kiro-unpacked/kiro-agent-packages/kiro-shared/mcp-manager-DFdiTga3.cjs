...
function resetApprovedEnvVars() {
  vscode__namespace.workspace.getConfiguration("kiroAgent").update(APPROVED_ENV_VARS_KEY, [], vscode__namespace.ConfigurationTarget.Global);
  hasShownWarningThisSession = false;
...
class MCPConnection {
  ...
  async connectClient(signal, fallbackToStreamableHttp = false) {
    ...
    try {
      ...
      } else if (error instanceof auth_js.UnauthorizedError && this.oauthRedirectServer && (transport instanceof streamableHttp_js.StreamableHTTPClientTransport || transport instanceof sse_js.SSEClientTransport)) {
...
const _MCPManagerSingleton = class _MCPManagerSingleton {
  ...
  static async reset() {
    ...
    try {
      ...
      ignore.mcpLogger.error(`Error removing connection for MCP server: ${e instanceof Error ? e.message : String(e)}`);
    }
