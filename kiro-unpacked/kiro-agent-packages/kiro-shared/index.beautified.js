...
import {
  F as FileSystemAccessError,
  i as isBadAuthIssue,
  M as MissingTokenError,
  a as MalformedTokenError,
  I as InvalidAuthError,
  b as SignInBlockedError,
  C as CanceledError,
  c as AbandonedError,
  d as InvalidUserInputError,
  e as AccessDeniedError,
  f as InvalidSSOAuthError,
  g as InvalidIdCAuthError,
  N as NetworkIssueError,
  U as UnexpectedIssueError,
  h as AuthError,
  j as UserEnvironmentError,
  k as UserFacingError
...
import {
  S as SocialAuthProvider,
  I as IDCAuthProvider
} from "./social-auth-provider-cN_RAEO7.js";
import {
  w as withSpan
} from "./span-Dkp1xmr6.js";
import {
  s as s2
} from "./span-Dkp1xmr6.js";
...
import {
  ...
  d as d3,
  e as e2
} from "./web-fetch-meter-CVTg62lw.js";
import {
  exec
} from "child_process";
import {
...
import {
  ...
  a as a5,
  g as g3
} from "./journey-tracker-BM6FRGIs.js";
import {
  u as u2
...
import {
  ...
  a as a7,
  u as u3
} from "./web-fetch-client-Cgle-lGV.js";
import {
  g as g5
} from "./machine-id-DDyBZGvP.js";
...
const _ProfileStorage = class _ProfileStorage {
  ...
  async writeProfile(profile) {
    ...
    try {
      ...
      logger.error("[ProfileStorage] Failed to write profile", {
        error
      });
    }
  }
  ...
  async readProfile() {
    try {
      ...
      logger.error("[ProfileStorage] Error reading profile", {
        error
      });
      return void 0;
    }
  ...
  async deleteProfile() {
    try {
      ...
      logger.error("[ProfileStorage] Error deleting profile", {
        error
      });
    }
  }
  ...
  async ensureDirectoryExists(dirUri) {
    try {
      ...
      logger.error("[ProfileStorage] Failed to create directory", {
...
class AuthProvider {
  ...
  async attemptRefreshIfCloseToExpiry() {
    try {
      ...
      const token = this.storage.readToken();
      if (isBadAuthIssue(error) && token && this.isAuthTokenExpiredWithinSeconds(token,
  ...
  async handleAuthError(error) {
    if (error instanceof AccessDeniedError || error instanceof MissingTokenError ||
      error instanceof MalformedTokenError || error instanceof InvalidAuthError ||
      error instanceof InvalidSSOAuthError || error instanceof InvalidIdCAuthError) {
