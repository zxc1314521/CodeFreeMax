...
const _ProfileStorage = class _ProfileStorage {
  ...
  async writeProfile(profile) {
    ...
    try {
      ...
      ignore.logger.error("[ProfileStorage] Failed to write profile", {
        error
      });
    }
  }
  ...
  async readProfile() {
    try {
      ...
      ignore.logger.error("[ProfileStorage] Error reading profile", {
        error
      });
      return void 0;
    }
  ...
  async deleteProfile() {
    try {
      ...
      ignore.logger.error("[ProfileStorage] Error deleting profile", {
        error
      });
    }
  }
  ...
  async ensureDirectoryExists(dirUri) {
    try {
      ...
      ignore.logger.error("[ProfileStorage] Failed to create directory", {
        dirUri: dirUri.toString(),
        error
      });
    }
  }
  ...
  validateProfile(data) {
    return !!data && typeof data === "object" && "arn" in data && "name" in data && typeof data.arn === "string" &&
      typeof data.name === "string" && data.arn.length > 0 && data.name.length > 0;
  }
};
...
function translateError(error) {
  return error instanceof ssoOidcClient.AuthError ? error : new ssoOidcClient.UnexpectedIssueError(
    "Auth provider: unexpected issue");
}
...
class AuthProvider {
  ...
  async attemptRefreshIfCloseToExpiry() {
    try {
      ...
      const token = this.storage.readToken();
      if (ssoOidcClient.isBadAuthIssue(error) && token && this.isAuthTokenExpiredWithinSeconds(token,
  ...
  async getTokenData({
    ...
    try {
      ...
      if (token.refreshToken && attemptRefresh) {
        return await span.withSpan(telemetry_definitions_index.TelemetryNamespace.Auth, "auth-provider.getTokenData",
          async () => {
  ...
  async handleAuthError(error) {
    if (error instanceof ssoOidcClient.AccessDeniedError || error instanceof ssoOidcClient.MissingTokenError ||
      error instanceof ssoOidcClient.MalformedTokenError || error instanceof ssoOidcClient.InvalidAuthError ||
      error instanceof ssoOidcClient.InvalidSSOAuthError || error instanceof ssoOidcClient.InvalidIdCAuthError) {
  ...
  async showInvalidSessionErrorMessage() {
    return span.withSpan(telemetry_definitions_index.TelemetryNamespace.Auth, "auth-provider.manual-error-resolve",
      async () => {
        ...
        if (action === "Refresh session") {
          try {
            ...
            void this.logoutAndForget();
            action = await vscode__namespace.window.showErrorMessage("We are unable to refresh your session.",
              "Login");
          }
        }
        if (action === "Login") {
...
class AuthProviderSession {
  constructor(provider) {
    ...
    this.account = {
      id: PROVIDER_ID,
      label: provider ? providerLabel[provider] : ""
    };
  }
}
class AuthProviderExtension {
  constructor(controller) {
    ...
    this.disposables.push(
      authProvider.onDidChangeLoginStatus(({
        token,
        isSignedIn
      }) => {
        if (isSignedIn) {
...
async function registerAuthProviderExtension(context) {
  ...
  const signInControllerRegistration = vscode__namespace.authentication.registerSignInController(PROVIDER_ID,
    signInController, {
      isInternalUser
    });
