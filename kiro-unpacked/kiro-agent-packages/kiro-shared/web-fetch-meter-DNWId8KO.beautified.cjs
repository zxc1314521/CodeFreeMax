...
const ChatUIMetrics = new telemetry_definitions_index.MetricReporter(telemetry_definitions_index.TelemetryNamespace
  .Feature, "chatUI");

function recordChatWebviewEvent(event) {
  try {
    if (event.value) {
      ChatUIMetrics.reportHistogramMetrics({
        [event.type]: event.value
      }, event.dimensions);
      return;
    }
    ChatUIMetrics.reportCountMetrics({
      [event.type]: 1
    }, event.dimensions);
  } catch {}
}
const PowersMetrics = new telemetry_definitions_index.MetricReporter(telemetry_definitions_index.TelemetryNamespace
  .Feature, "powers");

function recordPowersEvent(event) {
  try {
    PowersMetrics.reportCountMetrics({
      [event.type]: 1
    }, event.dimensions || {});
  } catch {}
}
...
function recordPowersHistogram(metricName, value, dimensions) {
  try {
    PowersMetrics.reportHistogramMetrics({
      [metricName]: value
    }, dimensions || {});
  } catch {}
}
const RemoteToolsMetrics = new telemetry_definitions_index.MetricReporter(telemetry_definitions_index.TelemetryNamespace
  .Feature, "remote_tools");

function recordRemoteToolsEvent(event) {
  try {
    RemoteToolsMetrics.reportCountMetrics({
      [event.type]: 1
    }, event.dimensions || {});
  } catch {}
}
...
function recordRemoteToolsHistogram(metricName, value, dimensions) {
  try {
    RemoteToolsMetrics.reportHistogramMetrics({
      [metricName]: value
    }, dimensions || {});
  } catch {}
}
class RemoteToolRecorder {
  constructor(toolName, additionalDimensions) {
    ...
    recordRemoteToolsEvent({
      type: "remoteToolInvoked",
  ...
  recordRejection() {
    const duration = performance.now() - this.startTime;
    recordRemoteToolsEvent({
      type: "remoteToolRejected",
      dimensions: this.dimensions
    });
    recordRemoteToolsHistogram("remoteToolDuration", duration, this.dimensions);
    recordRemoteToolsHistogram("userApproval", 0, {
      ...this.dimensions,
      action: "reject"
    });
  }
  ...
  recordAcceptance() {
    recordRemoteToolsEvent({
      type: "remoteToolAccepted",
      dimensions: this.dimensions
    });
    recordRemoteToolsHistogram("userApproval", 1, {
      ...this.dimensions,
      action: "accept"
    });
  }
  ...
  recordTrust() {
    recordRemoteToolsEvent({
      type: "remoteToolTrusted",
      dimensions: this.dimensions
    });
    recordRemoteToolsHistogram("userApproval", 2, {
      ...this.dimensions,
      action: "trust"
    });
  }
}
const WebFetchMetrics = new telemetry_definitions_index.MetricReporter(telemetry_definitions_index.TelemetryNamespace
  .Feature, "web_fetch");

function recordWebFetchEvent(event) {
  try {
    WebFetchMetrics.reportCountMetrics({
      [event.type]: 1
    }, event.dimensions || {});
  } catch {}
}
class WebFetchRecorder {
    ...
    if (result) {
      ...
      if (result.matchCount !== void 0) {
        recordWebFetchEvent({
          ...
          dimensions: {
            ...this.dimensions,
            matchCount: result.matchCount.toString()
          }
        });
  ...
  recordModeUsed(mode) {
    ...
    this.baseRecorder.recordHistogram("modeUsage", modeValue, {
      mode
    });
  }
  /**
  ...
  recordUnsafeUrlBlocked(url) {
    ...
    this.baseRecorder.recordHistogram("unsafeUrlAttempts", 1, {
      protocol
    });
  }
  /**
  ...
  recordUserAccepted() {
    ...
    this.baseRecorder.recordHistogram("userApproval", 1, {
      action: "accept"
    });
  }
  /**
  ...
  recordUserTrusted() {
    ...
    this.baseRecorder.recordHistogram("userApproval", 2, {
      action: "trust"
    });
  }
  /**
  ...
  recordUserRejected() {
    ...
    this.baseRecorder.recordHistogram("userApproval", 0, {
      action: "reject"
    });
  }
}
