import { K as d, T as j } from "./powers-D5ei-qtJ.js";
import { randomUUID as P } from "crypto";
import { MetricReporter as S } from "@kiro/shared";
...
function N(e, r, n) {
  return r = M(r), r in e ? Object.defineProperty(e, r, { value: n, enumerable: !0, configurable: !0, writable: !0 }) : e[r] = n, e;
}
function M(e) {
  var r = F(e, "string");
...
function O(e) {
  ...
  function r(n, ...t) {
    ...
    for (let a = 0; a < o.length; a++) {
      let s = o[a];
      if (c && (s = s.replace(/\\\n[ \t]*/g, "").replace(/\\`/g, "`").replace(/\\\$/g, "$").replace(/\\\{/g, "{")), i += s, a < t.length) {
...
function b(e, r) {
  ...
  if (n instanceof m.ZodNumber && typeof e == "string" && !isNaN(Number(e)) && e.trim() !== "")
    return { value: Number(e), wasCoerced: !0 };
  if (n instanceof m.ZodArray && Array.isArray(e)) {
    const t = n._def.type;
    let o = !1;
    return { value: e.map((c) => {
      const l = b(c, t);
  ...
  if (n instanceof m.ZodObject && e && typeof e == "object") {
    ...
    return { value: t, wasCoerced: o };
  }
  return { value: e, wasCoerced: !1 };
}
function W(e) {
  let r = "";
  const n = [];
  for (const t of e)
    ["text", "mention"].includes(t.type) && t.text && (r += t.text), t.type === "imageUrl" && t.imageUrl && n.push(t.imageUrl.url);
  return { entireMessage: r, imageBase64Urls: n };
}
function q() {
  const e = x.platform();
  switch (e) {
